...
for i in range(len(data)):
    ...
    # DV_grid[arr_idx, dep_idx] = vinf1[i]
    DV_grid[arr_idx, dep_idx] = vinf2[i]
...
# 塗りつぶし等高線
cp = plt.contourf(Dep_JD_grid, Arr_JD_grid, DV_grid, levels=levels, cmap='jet', extend='both')

# 等高線（線だけ）
cs = plt.contour(Dep_JD_grid, Arr_JD_grid, DV_grid, levels=levels, colors='black', linewidths=0.8)
...
# カラーバー
plt.colorbar(cp, label="DV (km²/s²)")

# 軸ラベルとか
plt.xlabel("Departure JD")
plt.ylabel("Arrival JD")
plt.title("Porkchop Plot: Earth to Mars (DV=20,25,30,35,40)")
plt.grid(True)

# 保存（オプション）
plt.savefig("porkchop_DV_fixed_levels2.png", dpi=300, bbox_inches='tight')
